...
/*   Updated: 2020/05/27 11:47:07 by mskinner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_next_line(int fd, char **line)
{
	...
	while ((readed_bytes = read(fd, buff, BUFFER_SIZE)) > 0)
	{
		...
		if (!(tmp = ft_strjoin(str[fd], buff)))
			return (ft_free(str + fd, line));
		if (str[fd])
			free(str[fd]);
		str[fd] = tmp;
		if (ft_strchr(str[fd], 10))
			return (ft_get_line_buff(str + fd, line));
	}
	if (readed_bytes < 0)
		return (ft_free(str + fd, line));
	return (ft_get_line_tail(str + fd, line));
}
